const CACHE_NAME = 'fifth-event-v1';
...
self.addEventListener('fetch', (event) => {
    ...
    // Пропускаем запросы к offline странице и health check
    const url = new URL(event.request.url);
    if (url.pathname === '/offline' || url.pathname === '/health') {
        return;
    }
    ...
    event.respondWith(
        fetch(event.request)
            ...
            .catch((error) => {
                console.log('Service Worker: Нет сети, проверяем кэш', error);
                
                // Если нет сети - проверяем кэш
                return caches.match(event.request).then((cachedResponse) => {
                    ...
                    // Если это навигация и нет в кэше - показываем офлайн страницу
                    if (event.request.mode === 'navigate') {
                        return caches.match(OFFLINE_URL).then((offlineResponse) => {
                            ...
                            // Если офлайн страница тоже не в кэше - показываем базовую заглушку
                            console.log('Service Worker: Оффлайн страница не в кэше, показываем заглушку');
                    ...
                    console.log('Service Worker: Ресурс не найден в кэше', event.request.url);
