<section id="schedule" class="timeline-section mx-auto max-w-4xl px-4 py-20 text-black">
    ...
    @if ($event)
        ...
        @if ($selectedDay)
                <a href="{{ route('ical.day', $selectedDay) }}" class="rounded bg-gray-100 px-3 py-1 text-sm text-gray-700 hover:bg-gray-200">Скачать день</a>
            @endif
            <a href="{{ route('ical.full', $event) }}" class="rounded bg-gray-100 px-3 py-1 text-sm text-gray-700 hover:bg-gray-200">Скачать всё</a>
        </div>
        ...
        <div class="mb-6 flex gap-2 overflow-x-auto border-b border-gray-200 pb-2">
            @foreach ($days as $day)
                ...
                <button type="button" wire:click="selectDay({{ $day->id }})"
                    class="rounded-t-lg border-b-2 px-4 py-2 text-sm font-medium transition {{ $isActive ? 'border-black bg-gray-100 text-black' : 'border-transparent text-gray-500 hover:text-gray-700' }}">
                    <span>{{ $day->label }}</span>
                    @if ($isPast && ! $isActive)
                        <span class="ml-1 text-xs text-gray-400">(завершён)</span>
                    @elseif ($isToday && ! $isActive)
                        <span class="ml-1 text-xs text-gray-500">сегодня</span>
                    @endif
                </button>
            @endforeach
        </div>
            ...
            <div class="relative pl-8 sm:pl-10" wire:key="day-{{ $selectedDay->id }}">
                ...
                @foreach ($selectedDay->events as $scheduleEvent)
                    ...
                    <div class="timeline-item relative pb-8 last:pb-0">
                        <div class="absolute left-3 z-10 h-3 w-3 -translate-x-1/2 rounded-full border-2 sm:left-4 {{ match ($status) { 'active' => 'animate-pulse border-gray-400 bg-black ring-4 ring-gray-200', 'past' => 'border-gray-200 bg-gray-300', default => 'border-gray-300 bg-white' } }}"></div>

                        <div class="ml-4 sm:ml-6">
                            <div class="mb-1 flex items-center gap-2">
                                <span class="rounded px-2 py-0.5 font-mono text-xs {{ match ($status) { 'active' => 'bg-gray-100 font-bold text-black', 'past' => 'bg-gray-100 text-gray-400', default => 'bg-gray-50 text-gray-500' } }}">
                                    {{ $start->format('H:i') }} - {{ $end->format('H:i') }}
                                </span>
                                @if ($status === 'active')
                                    <span class="animate-pulse text-xs font-bold text-black">Идёт сейчас</span>
                                @elseif ($status === 'past')
                                    <span class="text-xs text-gray-400">Завершено</span>
                                @endif
                            </div>

                            <div class="rounded-xl border p-3 transition-all sm:p-4 {{ match ($status) { 'active' => 'border-gray-300 bg-gray-50 shadow-md', 'past' => 'border-gray-100 bg-gray-50 opacity-60', default => 'border-gray-100 bg-white' } }}">
                                <div class="flex items-start gap-3">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <div class="relative mt-2" x-data="{ open: false }">
                                            <button @click="open = ! open" class="text-xs text-gray-600 hover:underline">Добавить в календарь</button>
