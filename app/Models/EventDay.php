...
class EventDay extends Model{protected $fillable=['event_id','date','label','description','sort_order','is_active'];protected function casts():array{return['date'=>'date','is_active'=>'boolean'];}public function event():BelongsTo{return $this->belongsTo(Event::class);}public function events():HasMany{return $this->hasMany(ScheduleEvent::class)->orderBy('sort_order')->orderBy('start_time');}}
